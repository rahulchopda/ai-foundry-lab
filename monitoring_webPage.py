...
def render_latency_section(metric_name: str = "Latency", unit: str = "ms"):
    ...
    <div style="display:flex;flex-wrap:wrap;margin:-0.4rem;">
        ...
        {_metric_block(f"Latest ({latest_time})", f"{latest:.2f} {unit}")}
        {_metric_block("Change", delta_str)}
        {_metric_block("Points", n_points)}
    </div>
    """
    ...
    table_html = f"""
    <table class="recent-table" style="width:100%;border-collapse:collapse;font-size:0.70rem;">
        <thead>
            <tr style="background:#e9f2fb;">
                <th style="text-align:left;padding:6px 8px;color:#0051a8;border-bottom:1px solid #d3dbe5;">Timestamp</th>
                <th style="text-align:right;padding:6px 8px;color:#0051a8;border-bottom:1px solid #d3dbe5;">Total ({unit})</th>
            </tr>
        </thead>
    ...
    <div class="ms-card" style="margin-top:1.3rem;">
        ...
        <div style="display:flex;flex-wrap:wrap;gap:1.25rem;margin-top:1rem;align-items:stretch;">
            ...
            <div style="flex:1 1 0%;min-width:220px;display:flex;flex-direction:column;">
                <div style="font-size:0.75rem;font-weight:600;color:#0051a8;margin:0 0 6px 0;">Recent</div>
